  ...
  if(transactions!=null && transactions.length==1 && new Date(transactions[0].outcome.timestamp).getFullYear()>=2018) {
    console.log("Found most recent profile transaction...");
      try {
        ...
        profileButtons();
        showProfilebox();
        if(address!=issuer1 && transactions[0].specification.memos[0].data.indexOf("[popup]")>0) showPopup(parseBBC(stripHTML(transactions[0].specification.memos[0].data)), symbol1+" by "+getIssuerName(issuer1, true));
      }
      catch(ex) {
