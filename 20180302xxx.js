    ...
    console.log("Found most recent profile transaction...");
      try {
        ...
        if(address!=issuer1 && transactions[0].specification.memos[0].data.indexOf("[popup]")>0) {
          ...
          showPopup(parseBBC(stripHTML(transactions[0].specification.memos[0].data))+"<br /><hr /><div style='text-align:center;'>"+$("#profileOnBlockchain").html()+"</div><hr /><div style='text-align:right;'>Last Updated "+formatDate(new Date(transactions[0].outcome.timestamp), true)+"</div>", symbol1+" by <span id='popupProfileName'>"+getIssuerName(issuer1, true)+"</span>");
        }
      }
      catch(ex) {
